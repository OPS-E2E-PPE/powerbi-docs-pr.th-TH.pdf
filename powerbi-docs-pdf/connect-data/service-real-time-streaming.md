...
บทความนี้แสดงให้เห็นว่า การตั้งค่าการสตรีมข้อมูลในเวลาจริงทำได้อย่างไรใน Power BI First, it's important to understand the types of real-time datasets that are designed to display in tiles (and dashboards), and how those datasets differ.
...
หลังจากสร้างรายงานโดยใช้ชุดข้อมูลแบบพุชแล้ว จะสามารถปักหมุดวิชวลไปยังแดชบอร์ดได้ บนแดชบอร์ด วิชวลจะปรับปรุงในเวลาจริง เมื่อใดก็ตามที่มีการปรับปรุงข้อมูล ภายในบริการ แดชบอร์ดจะทริกเกอร์การรีเฟรชไทล์ทุกครั้งที่ได้รับข้อมูลใหม่
...
ด้วย **ชุดข้อมูลการสตรีม** ข้อมูลยังถูกส่งไปยังบริการ Power BI ด้วยความแตกต่างที่มีนัยสำคัญ: Power BI จะเก็บข้อมูลลงไว้ในแคชเพียงชั่วคราวเท่านั้นซึ่งมีสิทธิ์ที่ข้อมูลเหล่านั้นจะหมดอายุลงได้ The temporary cache is only used to display visuals, which have some transient sense of history, such as a line chart that has a time window of one hour.
...
The only way to visualize a streaming dataset is to add a tile and use the streaming dataset as a **custom streaming data** source. ไทล์สตรีมแบบกำหนดเองที่มาจาก **ชุดข้อมูลการสตรีม** ถูกปรับให้เหมาะสำหรับการแสดงข้อมูลในเวลาจริงได้อย่างรวดเร็ว There is little latency between when the data is pushed into the Power BI service and when the visual is updated, since there's no need for the data to be entered into or read from a database.
...
With a **PubNub** streaming dataset, the Power BI web client uses the PubNub SDK to read an existing PubNub data stream. No data is stored by the Power BI service. เนื่องจากการเรียกใช้นี้มากจากไคลเอ็นต์เว็บโดยตรง คุณจะต้องแสดงรายการปริมาณข้อมูลไปยัง PubNub ตามที่ได้รับอนุญาต หากคุณอนุญาตการรับส่งข้อมูลขาออกที่ได้รับอนุมัติจากเครือข่ายของคุณ โปรดดูคำแนะนำในบทความการสนับสนุนเกี่ยวกับ [การอนุมัติการรับส่งข้อมูลขาออกสำหรับ PubNub](https://support.pubnub.com/hc/en-us/articles/360051496672)
...
ไทล์จาก **ชุดข้อมูลการสตรีม PubNub** ถูกปรับให้เหมาะสำหรับการแสดงข้อมูลในเวลาจริงได้อย่างรวดเร็ว Since Power BI is directly connected to the PubNub data stream, there is little latency between when the data is pushed into the Power BI service and when the visual is updated.
...
เมื่อสร้างชุดข้อมูลแล้ว ใช้ REST API เพื่อผลักข้อมูลโดยใช้ [**PostRows** API](/rest/api/power-bi/pushdatasets/datasets_postrows)
...
Azure Stream Analytics ใช้ Power BI REST Api ในการสร้างสตรีมข้อมูลของผลลัพธ์ไปยัง Power BI เมื่อ *defaultMode* ตั้งค่าเป็น *pushStreaming* ซึ่งทำให้ชุดข้อมูลสามารถใช้ประโยชน์จากทั้งการ **พุช** และการ **สตรีม** เมื่อมีการสร้างชุดข้อมูล Azure Stream Analytics จะตั้งค่าสถานะ **retentionPolicy** เป็น *basicFIFO* ด้วยการตั้งค่าดังกล่าว ฐานข้อมูลที่สนับสนุนชุดข้อมูลแบบพุชจะจัดเก็บแถว 200,000 แถวและแถวใดจะถูกวางในแบบเข้าก่อนออกก่อน (FIFO)
...
ถ้าคุณต้องการให้ Power BI จัดเก็บข้อมูลที่ส่งผ่านสตรีมข้อมูลนี้ เปิดใช้งาน *การวิเคราะห์ข้อมูลในอดีต* แล้วคุณจะสามารถสร้างรายงานและวิเคราะห์ข้อมูลที่รวบรวมจากสตรีม คุณยังสามารถ[เรียนรู้เพิ่มเติมเกี่ยวกับ API](/rest/api/power-bi/) ได้
...
> [!WARNING]
> สำหรับการสตรีมชุดข้อมูลที่สร้างโดยใช้ UI ของบริการ Power BI เจ้าของชุดข้อมูลจะได้รับ URL ซึ่งมี **คีย์ทรัพยากร** คีย์นี้อนุญาตให้ผู้ร้องขอพุชข้อมูลลงในชุดข้อมูลโดยไม่ต้องใช้โทเค็นผู้ถือ Azure AD OAuth ดังนั้นโปรดคำนึงถึงผลกระทบของการมี **คีย์ลับ** ใน URL เมื่อทำงานกับชุดข้อมูลและวิธีการประเภทนี้
...
3. ใส่ชื่อสำหรับชุดข้อมูลของคุณ จากนั้นวางค่าต่อไปนี้ลงในหน้าต่างที่ปรากฏขึ้น แล้วเลือก **ถัดไป**:
   ...
   **คีย์สมัครใช้งาน:** *sub-c-5f1b7c8e-fbee-11e3-aa40-02ee2ddab7fe*

   **แชนแนล:** *pubnub-sensor-network*
...
สำหรับชุดข้อมูลแบบพุช เมื่อคุณมีการประทับเวลาใน schema คุณสามารถลองสร้างวิชวลรายงานด้วยตัวกรอง N ล่าสุด
...
ชุดข้อมูลแบบพุลและแบบไฮบริดสามารถเชื่อมต่อใน Power BI Desktop ได้ ชุดข้อมูลการสตรีมอื่นๆ ไม่สามารถเชื่อมต่อใน Power BI Desktop ได้
...
การสร้างรูปแบบทำไม่ได้ในชุดข้อมูลการสตรีม เนื่องจากข้อมูลไม่ได้ถูกเก็บถาวร สำหรับชุดข้อมูลแบบพุช คุณสามารถใช้การสร้างชุดข้อมูล REST API เพื่อสร้างชุดข้อมูลที่มีความสัมพันธ์และหน่วยวัดและ/หรือใช้ REST Api ของตารางอัปเดตเพื่อเพิ่มหน่วยวัดไปยังตารางที่มีอยู่ 
...
5. งาน Azure Stream Analytics มีข้อมูลไหลผ่านหรือไม่? ชุดข้อมูลจะถูกสร้างขึ้นเมื่อมีการส่งข้อมูลเท่านั้น
...
* [ภาพรวมของ Power BI REST API กับข้อมูลในเวลาจริง](/rest/api/power-bi/)
